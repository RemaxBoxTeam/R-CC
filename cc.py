...
def logo():
    figlet = Figlet(font="standard").renderText("Remax Box")
    return (gn + figlet)
print (logo())
print (bl + "[-] Powered by Remax Box Team ")
print (gn + "[+] Made By Maximum Radikali")
print (cy + "[=] R-CardTools Version : 1.0")
...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram Channel : @RemaxBoxTeam" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
if opr == "1":
    ...
    type = input(gg + "[-] Visa\n[-] MasterCard\n[-] American Express\n[-] Discover\n[-] JCB\n\n[$] Please enter type of card : ")
    print (ccvalidator(number , type))
